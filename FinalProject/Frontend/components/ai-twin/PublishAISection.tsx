  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-6">
        <h2 className="text-xl font-semibold text-white mb-4">Kiểm tra điều kiện xuất bản</h2>
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            {isPromptValid ? <CheckCircle size={18} className="text-green-400" /> : <AlertCircle size={18} className="text-red-400" />}
            <span className={isPromptValid ? 'text-white' : 'text-red-400'}>System prompt hợp lệ (dưới 2.000 ký tự)</span>
          </div>
          <div className="flex items-center gap-3">
            {isKnowledgeValid ? <CheckCircle size={18} className="text-green-400" /> : <AlertCircle size={18} className="text-red-400" />}
            <span className={isKnowledgeValid ? 'text-white' : 'text-red-400'}>Cơ sở kiến thức dưới 15.000 ký tự</span>
          </div>
          <div className="flex items-center gap-3">
            {hasKnowledge ? <CheckCircle size={18} className="text-green-400" /> : <AlertCircle size={18} className="text-red-400" />}
            <span className={hasKnowledge ? 'text-white' : 'text-red-400'}>Có ít nhất một mục trong cơ sở kiến thức</span>
          </div>
          <div className="flex items-center gap-3">
            {hasGuardrails ? <CheckCircle size={18} className="text-green-400" /> : <AlertCircle size={18} className="text-red-400" />}
            <span className={hasGuardrails ? 'text-white' : 'text-red-400'}>Đã bật các quy tắc an toàn</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-6 flex flex-col justify-between">
          <div>
            <h3 className="text-lg font-semibold text-white mb-2">Lưu & Huấn luyện</h3>
            <p className="text-sm text-white/50 mb-6">
              Lưu cấu hình hiện tại của bạn dưới dạng bản nháp hoặc tạo dữ liệu mô hình AI mà không công khai.
            </p>
          </div>
        ...
        <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-6 flex flex-col justify-between">
          <div>
            <h3 className="text-lg font-semibold text-white mb-2">Truy cập công khai</h3>
            <p className="text-sm text-white/50 mb-6">
              Xuất bản AI Twin của bạn để khách truy cập tương tác trên trang cá nhân công khai.
            </p>
          </div>
          <div className="flex gap-3 mt-auto">
            <button
            ...
            {config.status !== 'AI Draft' && config.status !== 'AI Training' && config.status !== 'Prompt Too Long' && (
              ...
              >
                ...
                {config.isPublicEnabled ? 'Tắt công khai' : 'Bật công khai'}
              </button>
            )}
          </div>
        </div>
