...
Dựa vào các tài liệu dự án mà tôi đã nạp (như `PRD.md`, `Test_Plan_Guideline.md`, `api-tree.md`), nhiệm vụ của bạn là phân tích và thiết kế Kịch bản kiểm thử (Test Cases) chi tiết cho:
...
- **Edge Cases & Core Risks:** Đặc biệt chú trọng các rủi ro đã được định nghĩa trong `Test_Plan_Guideline.md` (Ví dụ: Timeout từ API bên thứ 3, Rate Limit/Spamming, AI Hallucination, Lỗi phân quyền).
...
1. **Hiển thị Checklist đánh giá:**
   ...
   - `[ ]` Đã bao phủ Edge Cases/Core Risks từ Test_Plan_Guideline.md.
   - `[ ]` Format output chuẩn xác (phân tách Frontend/Backend rõ ràng).

2. **Sinh đoạn text Ghi Log (Cập nhật tiến độ):**
   Hãy sinh ra một đoạn text Markdown log ngắn gọn để tôi dễ dàng copy dán vào file `Test case/Test_Logs/Test_Logs.md`:
   **Log Format:**
